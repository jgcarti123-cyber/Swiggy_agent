...
**What it does.** The user uploads a screenshot of another quick-commerce app's cart (Zepto, Blinkit, Instamart, BigBasket — whatever), and Pantry Pal reproduces that cart on Instamart: exact same items where available, three alternatives to pick from where not. This is the only feature that needs to *see* an image — everything else in the app is text.
