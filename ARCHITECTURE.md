...
**Search per question, not once per item — a real bug, found by the user, not a hypothetical.** The first version searched **once** per spinId (`"${name} ${brand} ingredients nutrition review"`) and cached that single result for every question asked about the item, on the theory that the underlying product doesn't change mid-conversation so re-searching per question would be pure waste. Confirmed live this was wrong in practice: asking "does this have caffeine in it?" about Coca-Cola Zero got a flat, unhelpful refusal — the one generic search never happened to surface caffeine content, and the system prompt told the model to only use what it was given, so it wouldn't reach for the (correct, well-known) answer it actually had. Fixed by keying `itemSearchCache` on **`(item, exact question)`** and building each search query from the product identity **plus the literal question** — re-verified live, the identical question now returns "Coca-Cola Zero contains caffeine — about 34mg per 300ml can" with three real, specific sources (caffeineinformer.com among them), because a targeted search actually finds caffeine-specific content where a generic one didn't. Re-asking the exact same question about the same item still hits the cache (no repeat spend); a differently-worded question searches fresh, which is the intended trade — more Tavily calls, meaningfully better answers.
