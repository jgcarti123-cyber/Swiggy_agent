...
**Auto-pick logic is shared with the recipe flow (§6.13) and was tuned after live testing surfaced two distinct bugs**, both fixed at the shared `sortForBestPick`/`orderBestFirst` layer (§6.13 has the full writeup) rather than duplicated per flow: Swiggy's own relevance order was being overridden by a price-ascending sort (chicken masala outranking real chicken), and separately, the top-3-relevant options weren't reliably surfacing the cheapest one to auto-add. Both apply identically here since `confirmImportDirect` calls the same two functions.
