...
| LLM | Groq (`groq-sdk`), text model `openai/gpt-oss-120b` + vision model `qwen/qwen3.6-27b` | Text model powers restaurant-discovery judgment calls and free-text interpretation in the Instamart chat (see §5); the vision model is used only by the screenshot-import feature (§6.15), since the text model has no image input |
...
- **`callSwiggyTool(serverUrl, name, args)`** is the single chokepoint every tool call goes through. It calls `client.callTool({ name, arguments })`, then unwraps the MCP response shape:
  ...
  - If `result.structuredContent` exists, or the text content parses as JSON, the result is passed through `unwrapEnvelope()` before being returned — see below, this is not optional as of a live-confirmed change in Swiggy's own response behavior.
  - Falls back to raw text if the content isn't JSON.
- **Swiggy's documented generic envelope (`{success, data, message}`) started actually appearing live, inconsistently, and every caller in this codebase had been built and tested against the unwrapped shape.** Swiggy's docs have always described this envelope (confirmed by directly fetching the live docs early in this project), but live inspection of `search_products`, `get_cart`, etc. consistently found the real payload arriving flat — `raw.products`, `cart.items` — never nested under `.data`, which is what §2.5 and every downstream function was built against. Confirmed live in a later session that this stopped being reliable: a "banana chips" search had 20 real, relevant results sitting under `raw.data.products` while the app checked the top-level `raw.products`, found nothing, and reported a false "couldn't find anything" — and `get_cart` for an empty cart returned `{success:true, data:{items:[], cartAbsent:true}, message:"..."}` instead of throwing the documented empty-cart error, so `getCartOrEmpty()`'s error-message-matching special case never even ran, and the raw wrapped object reached the frontend, which fell through to its `<pre>{JSON.stringify(cart)}</pre>` last-resort branch — visible to the user as a raw JSON dump where a normal "cart is empty" UI should have been. Repeated identical calls showed the wrapping isn't constant either (a brand-choice screen had worked moments earlier in the same session), so this reads as Swiggy's server being inconsistent about which shape it returns, not a clean migration. `unwrapEnvelope()` normalizes both shapes at the one chokepoint every tool call already passes through: if a parsed result looks like `{success: boolean, data: ..., ...}`, it unwraps to `.data` (and throws a `SwiggyToolError` from `.message` if `success` is `false`, mirroring how `result.isError` is already handled); otherwise the value passes through unchanged. Every caller downstream — `foodClient.js`, `instamartClient.js`, and everything built on them — now sees a consistent flat shape regardless of which one Swiggy actually sent for that particular call.
...
- **A real, low rate-limit ceiling (8000 TPM on the free tier)** that shapes design decisions directly: keeping `max_tokens` low, batching data into as few calls as possible, the "fail closed" fallback behavior described in §4.2, and — the deeper version of this problem, found later while profiling Insta-nt — the ceiling isn't just "one big request can exceed it," it's a **rolling per-minute cumulative quota**: a session making several back-to-back completions can get progressively throttled as it climbs toward the ceiling, even when every individual request is a very reasonable size. Measured directly: identical-sized completions within one session went 3.7s → 19.7s → 29s → 26.8s → 41.4s, a pattern that tracks cumulative usage, not any single request's cost. See §6.3 — this is *why* the fix for Insta-nt's slowness was "make fewer completions happen at all," not just "make each one smaller."
...
## 6. Feature 2: Insta-nt (`backend/src/agent/`)
...
- **The system prompt is the backstop** for whatever the pre-gate deliberately lets through (its recall is intentionally partial). `SYSTEM_PROMPT`'s first rule is now a hard scope constraint: only groceries/cart/usuals/recipes/checkout, refuse everything else in one short sentence, never answer trivia even if known, never call a tool for an off-topic request. So a subtle off-topic message the pre-gate misses ("recommend a movie") still gets refused — at the cost of one small completion, never a real answer. Verified live end-to-end: capitals/weather/math/coding/chit-chat refused (pre-gate at zero cost, or model backstop for the subtle ones), while "add milk" / "what milk do you have" / "i need stuff for breakfast" all pass through untouched.

### 6.3 The incident that drove this: measuring and fixing a 129-second guided add

The trigger was a direct user report: Insta-nt felt slow and was burning through the Groq quota fast. Rather than guess, the fix started with instrumentation — `toolLoop.js` now logs, per completion: wall-clock time, request size in raw characters, and Groq's own `usage` numbers (prompt/completion/reasoning tokens). Running the actual guided flow ("add milk" → "Amul" → click Add) through the real code with this logging produced:
...
**Some items report in-stock but Swiggy rejects them anyway, unpredictably.** Confirmed live and repeatedly: a specific fresh-meat item (`isInStockAndAvailable: true`, a fully-populated product record) still failed `update_cart` with `"No valid items in cart"` on some attempts and succeeded on others, with no code change in between and no way to predict it from the search response. This can't be filtered client-side the way the previous paragraph's case can — there's no signal to filter on ahead of time. The response is UX, not prevention: `friendlyCartError()` maps Swiggy's raw multi-line error text (report IDs, support email) to a short, actionable phrase, telling apart a genuine out-of-stock rejection ("it's out of stock right now — try another option") from an unrelated unpredictable rejection ("Swiggy rejected this item right now — try again in a bit, or pick something else") — collapsing both into one misleading "try a different size or brand" message was tried first and was actually wrong for the out-of-stock case. A failed add never blocks or cascades into the next one (verified: adding a different, working item immediately after a failed one succeeds normally). It's no longer purely reactive UX either: `addItemDirect` now calls `markSpinOutOfStock(spinId)` whenever the failure is attributable to that item (not a cart-level problem), which forces `inStock: false` on every later re-cache of that product for the rest of the process's life — regardless of what Swiggy's search claims — and retroactively greys out any already-rendered card for it in the live chat transcript via `outOfStockSpinId` in the response, so the user is never invited to retry a proven dead end.

**A `get_cart` failure over one item's stock state can "poison" the cart.** Every subsequent read — including the read every deterministic mutation starts with — fails identically until the cart is cleared. Confirmed live: `clear_cart` succeeds even while `get_cart` is stuck this way, and reads work normally again immediately after — in one session this was observed to self-resolve only because an unrelated real cart write (a recipe confirm) happened to clear it, leaving the Cart panel showing a dead-end error the whole time until then. `instamartClient.getCartOrEmpty()` now recognizes this exact message pattern ("partially available", the same one `friendlyCartError()` matches) and auto-recovers once — clear, then re-read — instead of just surfacing the error and waiting for some unrelated action to fix it. Deliberately narrow: only this specific phrase triggers the auto-clear, so a read failure for some other reason is never treated as license to wipe the cart.
...
**The plural tolerance itself had a bug, found live via the recipe flow:** `tokenVariants` originally only stripped a trailing "s" ("tomatoes" → "tomatoe"), which never matches "Tomato" in a product name — so a "tomatoes" ingredient failed the relevance filter for *every* result, tripped the safety valve below, and fell back to Swiggy's raw unfiltered order, where a carrot outranked the tomatoes. Now also strips a trailing "es" ("tomatoes" → "tomato", "boxes" → "box") before falling back to the "s"-only strip.
...
**Safety valve — now proportional, after a live category/synonym failure.** Originally the valve was all-or-nothing: only skip the filter if it removed *every* result. That missed a whole class of failure found live: a synonym/category query where a *tiny minority* of results literally match, so the filter keeps only those and silently drops the rest. Concretely, "underwear" returns 20 real products (Pepe Jeans, Jockey, XYXX, Dollar…) but exactly **1** literally contains the word "underwear" (a DaMENSCH pack) — the other 19 are named "trunk"/"brief"/"boxer". The old valve saw `relevant.length === 1 > 0`, kept just that one, and showed a single brand; meanwhile "underpants" (a true synonym) had **0** literal matches, tripped the valve, and correctly showed all 20 — the exact discrepancy a user reported ("these mean the same thing, why different results?"). The valve now also keeps the unfiltered set when the literal-match subset is a small fraction (`RELEVANCE_MIN_FRACTION`, 25%, on lists of 8+), which is the fingerprint of the keyword filter being the weak link on a category term. Food noise is unaffected because it fails the *opposite* way: for "chicken", most results genuinely contain "chicken" (well above 25%), so the filter still runs and still removes the "Too Yumm Protein Chips" outlier. This is deliberately a heuristic threshold, not a synonym dictionary — a synonym map (underwear↔trunk↔brief, cold-drink↔soft-drink, …) was considered and rejected as unbounded maintenance for a single-user tool, where "trust Swiggy's own relevance when our keyword filter is clearly the weaker signal" generalizes without a word list.

**A related scope bug this surfaced:** the off-topic guardrail (§6.2) had told the model its scope was "grocery shopping," and the model started *refusing* "add underwear" as non-grocery before the search ever ran. But Instamart is a general quick-commerce store (apparel, personal care, electronics, pet supplies…), not just food. The `SYSTEM_PROMPT` SCOPE rule was widened to say so explicitly — search for any plausible product, only refuse genuinely non-shopping questions (trivia/math/coding/chit-chat). The deterministic pre-gate never had this problem (it passes anything with a shopping verb like "add"); it was purely the model over-reading the word "grocery."
...
**A second, more dangerous failure mode found live: `update_cart` can report success while silently NOT including an item.** Not the already-documented case (§6.5) where Swiggy rejects the whole batch with an error — this is a normal 200-ish response, no error thrown, yet the cart afterward is missing one item Swiggy never complained about. The first version of `addUsualsBestEffort` (and `addItemDirect`) trusted "the call didn't throw" as proof every item landed, so a 4-item reorder could report "Added 4 usual items to your cart ✓" while the cart genuinely held 3 — confirmed live and reproducible for a specific item at a specific store (a Coca-Cola product that `search_products` itself reports `isInStockAndAvailable: true` for). The fix, in both `addUsualsBestEffort` and `addItemDirect`: never trust the absence of a thrown error — after every `update_cart` call, check the *returned cart's actual contents* (`keysInCart`, matching by `spinId:skuId`) against what was requested. Anything missing gets one retry (mirroring the existing "retry once" tolerance `instamartClient.js` already gives thrown `update_cart` failures — this just extends that same tolerance to a failure mode that layer can't see because nothing throws), and `added`/`failed` in the final reply are always computed from that last real cart state, never assumed from whether a call succeeded or failed.
...
### 6.13 The recipe flow — "order things for making biryani"

**The shape of the problem.** A recipe request needs (a) real food knowledge (what does biryani actually require?) — genuinely a model job — and (b) a dozen product searches, ranking decisions, and cart adds — none of which are. Given §6.3's lesson (per-turn LLM calls were the latency/cost problem), the original design goal was: exactly **one** completion for the proposal step, everything else deterministic. That held until §6.16 added web-search grounding — the proposal step is now two completions, a deliberate, user-approved trade of a bit more latency for a real recipe instead of pure model recall (see §6.16 for why and how it fails open).

**Step 1 — the model proposes, inside its tool call.** A new LLM tool, `propose_ingredients(dish, ingredients[])`, has the model generate the essential-ingredient list *in the tool-call arguments themselves* — there is no second "planning" completion, no search results fed back for judgment. The system prompt constrains the list to short, generic, brand-free Instamart search terms, and specifically to **Indian grocery names, not US English** — "curd" not "yogurt", "coriander leaves" not "cilantro", "capsicum" not "bell pepper", "paneer" not "cottage cheese". Confirmed live this mattered: "yogurt" as a search term surfaces sweetened Greek/flavoured tubs (Epigamia, Pride of Cows) because that's what's branded "yogurt" on Instamart, while "curd" correctly returns plain dahi — the item Indian recipes actually mean. `executeTool` sanitizes (trim/dedupe/cap 16) — this list is now only a **fallback** (§6.16) — and ends the loop via the `__endLoop` sentinel (§6.1) with a new branch kind, `ingredients` — rendered as an editable checklist (chips with ×, an add-ingredient input, a Confirm button). The user, not the model, owns the final list.
...
**Swap is atomic** (`swapRecipeItemDirect`, `POST /recipe-swap`): one `update_cart` carries both the removal of the old pick and the addition of the new one, so a rejected add (§6.5's unpredictable rejections — observed live during verification with fresh chicken) leaves the cart *unchanged* rather than half-swapped, surfaces a friendly error, and keeps the UI marker on the item that's genuinely still in the cart. A successful swap updates the held `lastRecipeEntry`'s group marker in place (the transcript entry object, by reference — same pattern as the stepper's no-chat-spam rule: a swap is a cart mutation, not a message). The checklist entry itself is marked `confirmed` server-side on first confirm, so a page reload renders it inert instead of offering a second Confirm that would double-add everything.

**Known limitation, revised after the sort fix above — not fully closed.** The original symptom (chicken → chicken masala, mint leaves → POLO Breath Mint) was mostly the price-sort bug now fixed by `sortForBestPick`. But re-verified live after that fix: "mint leaves" now avoids the breath-mint mismatch but still auto-picks "Curry Leaves & Green Chilli" — a genuinely different bug underneath, exposed now that the sort no longer masks it. `filterRelevantProducts` (§6.10) keeps a product if **any** significant query token matches, and "leaves" alone is enough to admit an unrelated curry-leaves combo product into the relevant set, where it then wins on price. Fixing this properly (e.g. requiring more of the query's tokens to match, or weighting a full-phrase match higher) hasn't been done — it's a real gap, not yet closed, and the swap rows remain the practical recovery path.

### 6.14 Requested-quantity matching and explicit-brand auto-detect

Two related "the app should have known that" fixes, both deterministic, both zero added Groq calls, both applied inside `runSearchAndBranch` before branching decides ask-vs-show.

**Requested-quantity matching.** "Add 100g of paneer" was searching for "paneer" and showing whatever pack sizes Swiggy happened to return, ignoring the "100g" entirely. The fix parses a quantity out of the query (`parseQuantityFrom` — a shared regex-based parser also used on each variant's own `quantityDescription`, so both sides are read the same way): a number plus a weight/volume/count unit (`g`/`kg`, `ml`/`l`, `piece(s)`/`dozen`, with common spellings), normalized to `{ family: "weight" | "volume" | "count", value }` in a base unit (grams, ml, or count) so "1kg" and "1000g" compare equal. `filterByRequestedQuantity` then narrows `raw.products[*].variations` to only the variants whose own size matches, **before** `distinctBrands`/`flattenVariants` ever run — so a brand that only sells a different size is never offered as a choice, and the resulting `lastSearchContext` (used by "show more") only ever pages through matching sizes. A multipack like `"65 g x 2"` is parsed on its leading number (the per-pack size, `65 g`) and the `x 2` is ignored — "100g" is read as the pack the user will see and hold, not a multipack's total, a deliberate simplification documented as a trade-off rather than solved with pack-total arithmetic that would need to guess the user's actual intent.

If nothing matches the requested size, the same "don't report a false empty result" principle used elsewhere (§6.4's `forceBrand` fallback, §6.10's relevance-filter fallback) applies: fall back to every size found, and prepend a `sizeNotice` ("Couldn't find a 100 g pack — here's what's available:") to whichever outcome follows — the plain product-card intro, or the brand-choice question, whichever this particular search would have shown anyway. The system prompt (`SYSTEM_PROMPT`) and the `search_products` tool schema both now explicitly tell the model to keep a stated weight/size in its query verbatim — the parser only ever sees what reaches `runSearchAndBranch`, so if the model paraphrased "100g" away, there'd be nothing to match on. This is the one place the fix depends on model cooperation rather than being fully self-contained in code, and it's a cheap, low-risk ask (one sentence added to an existing instruction) rather than a second completion.

**Clothing-size matching — a second, different dimension in the same field.** "show me only size L underpants" was showing every size, because a garment size (S/M/L/XL) isn't a weight/volume/count the quantity parser understands — yet Swiggy encodes it in the *same* `quantityDescription` string, as a suffix after the count: `"1 L"` = one unit, size Large; `"2 XL"` = two, size XL; `"1 M x 2"` = size M, multipack. `filterByRequestedSize` runs right after `filterByRequestedQuantity`, same fall-back-with-a-`sizeNotice` contract, narrowing to variants whose parsed size suffix (`variantSize`, anchored *after* the leading count so a `"500 ml"` unit can't be misread as a size) equals the requested one.

The load-bearing subtlety is that **"L" is Large *or* Litre**, and the two must never be confused. `parseSizeFrom` treats a lone letter as a garment size only under a clear signal — the word "size" ("size L" / "L size"), a full size word ("small"/"medium"/"large"), an unambiguous token (XS/XL/XXL), or an apparel word in the query (underwear/trunk/brief/socks/…). So "1 l milk" stays a volume (owned by the quantity parser, which requires a leading number+unit), "small onions" and "large pizza" get no size filter at all (no apparel, no "size"), while "size L underpants" and "L underpants" both resolve to Large. The two size filters are mutually exclusive per query in practice (a query states a weight *or* a garment size, not both), so whichever the query actually expressed is the one that narrows the results; both run before brand grouping so, exactly like the quantity filter, a brand that only stocks other sizes is never offered as a choice. Verified live end-to-end: "size L underpants" returned only `"1 L"`/`"2 L"` variants across eight brands, while "1 litre milk" still filtered by volume (its notice read "Couldn't find a 1 l pack", not "size L").

**Explicit-brand auto-detect.** Confirmed live and reported by the user: asking "order SuperYou 10g Protein Wafer Bar — Which brand of SuperYou 10g Protein Wafer Bar would you like?" is nonsensical — the brand was right there in the request. This happens because Swiggy's fuzzy search still fans a brand-qualified query out into other brands (already known — §6.4's comment on `flattenVariants` — "amul milk" returns Chitale, Gokul, etc. too), and the existing "2+ distinct brands found → ask" rule had no way to notice the user had already answered.

`detectExplicitBrand(query, brands)` is checked — only when the ask would otherwise fire (2+ candidate brands, post quantity-filter) — for whether the query's own words contain exactly one of those candidate brand names as a **contiguous token match** (word-boundary: the brand's own words appearing in order, not a raw substring check, so a short brand name like "PHAB" can't accidentally match inside an unrelated word). If exactly one candidate is named, it's treated exactly like an explicit `forceBrand` answer — the ask is skipped, `pendingBrandChoice` is never set, and the results go straight to product cards for that brand.

**The single-match requirement is the whole safety net.** If the query happens to name two real brands that overlap textually — e.g. "Godrej Real Good chicken" contains both the candidate "Godrej Real Good" *and* the candidate "Real Good Chicken" as contiguous substrings of its own token list — `detectExplicitBrand` returns `null` and the normal ask still fires. Verified directly (not just reasoned about): a small standalone test harness ran both the quantity parser (14 cases: unit conversions, multipacks, decimal amounts, no-match queries) and the brand detector (7 cases including this exact overlap scenario) against the actual functions before shipping. This detector can only ever remove a question that had exactly one right answer already stated — it can never suppress a genuinely ambiguous one, which is what keeps it safe to apply unconditionally rather than needing a confidence threshold or a fallback LLM check.

### 6.15 Import from a screenshot — the one genuinely-vision feature

**What it does.** The user uploads a screenshot of another quick-commerce app's cart (Zepto, Blinkit, Instamart, BigBasket — whatever), and Insta-nt reproduces that cart on Instamart: exact same items where available, three alternatives to pick from where not. This is the only feature that needs to *see* an image — everything else in the app is text.

**Why a second model.** The app's default model (`openai/gpt-oss-120b`) has no image input, so this uses a separate vision-capable model (`config.groqVisionModel`, default `qwen/qwen3.6-27b`). It's isolated in its own module (`agent/imageImport.js`) with one job — `extractItemsFromImage(dataUrl)` returns `[{ name, size, quantity }]`. The prompt is deliberately narrow: product line items only (brand + generic product name, pack size, quantity from the stepper), everything else on a busy checkout screen — prices, "you saved ₹164" banners, delivery ETA, UPI offers, address, totals, buttons — explicitly ignored. `response_format: json_object`, `temperature: 0`, and a salvage-the-first-`{…}`-block fallback in case the model wraps its JSON in prose. Verified live against a synthetic test screenshot: a 3-item cart came back as clean, search-ready terms with correct sizes and quantities.

**Groq retired the original vision model without notice — a real production break, caught by inspecting the live server, not from a changelog.** This app originally used `meta-llama/llama-4-scout-17b-16e-instruct`, confirmed present in both Groq's vision docs and the live models table when first adopted. At some later point Groq removed it entirely (it no longer appears in a live `GET /v1/models` call), and every import attempt started failing with a generic user-facing "I couldn't read that image" — the actual cause, `404 model_not_found`, was being caught and silently discarded with no server-side log at all, so the failure was invisible until directly reproduced and inspected. Two fixes: (1) `importImageDirect`'s catch block now `console.error`s the real Groq error before falling back to the friendly message, so this class of failure is never silent again; (2) `config.groqVisionModel` now defaults to `qwen/qwen3.6-27b`, confirmed via a direct `/v1/models` call to be Groq's current — and only — vision-capable model. It defaults to a "thinking" mode that prepends a `<think>...</think>` reasoning block before the real answer; confirmed live this was consuming the entire `max_tokens` budget on reasoning text alone and returning truncated or no JSON. Fixed by passing `reasoning_effort: "none"` on this call — confirmed live this returns the bare JSON object with no preamble. Since Groq's model lineup shifts (this is the second model rename this project has hit — see the text-model note in the table above), treat this as a component worth a live `/v1/models` check if the import feature ever silently stops working again.

**Vision is the *only* model step; matching and adding are the same deterministic pipeline as everything else.** After extraction there are zero further LLM calls. The flow deliberately mirrors the recipe flow (§6.13) — a checklist to review, then a deterministic confirm — and reuses its rendering wholesale:

- **Review before add** (the user's explicit choice over auto-add — OCR can misread). The extracted items render as an editable `import` checklist: a quantity stepper and a remove × per row. Nothing touches the cart until Confirm. The checklist is marked `confirmed` (client state + server transcript) on confirm so a reload can't re-import.
- **Confirm** (`confirmImportDirect`, `POST /import-confirm`) fans out with `mapWithConcurrency(…, 3)` and runs each item through the identical pipeline a typed order uses: relevance filter (§6.10), go-to cross-reference (§6.9), stock/price sort (§6.5). Then the import-specific logic: **strict size matching** (the user's choice). Using §6.14's `parseQuantityFrom`/`quantityMatches`, an item counts as an *exact match* only if a same-size in-stock variant exists; that variant is auto-added at the screenshot's quantity. A size-only mismatch (same product, wrong pack) or no match at all is **not** auto-added — it shows up to 3 alternatives instead, so the user chooses. Exact matches are added in one real-cart-verified batch (§6.5's silent-drop lesson — the "added N" count comes from the actual cart).
- **Rendering reuses the `recipe` message type verbatim** — the group shape (`{ ingredient: label, quantity, options, addedSpinId, note }`) is the same, so the compact swap-rows and `/recipe-swap` handler need no import-specific variant. Two small extensions made it fit: `swapRecipeItemDirect` now takes a `quantity` (so adding a not-found item's chosen option uses the screenshot's quantity, not 1 — recipes pass nothing and default to 1), and each group carries a `note` ("Exact match added" in green, or "No exact match — pick one:") the row renderer shows.

**Two plumbing details that matter.** The screenshot is downscaled and JPEG-compressed **in the browser** (`downscaleImage`, canvas, long edge ≤ 1600px, quality 0.82) before upload — a phone PNG is several MB and Groq's vision endpoint caps a base64 image at 4MB; the compressed result lands well under 1MB. And `express.json`'s body limit is raised from its 100kb default to 8mb (safe headroom for the data URL) — a one-liner, fine for a single-user local app.

**Privacy note, inherent to the feature:** the screenshot is sent to Groq's API for extraction — the same third party already processing the user's typed grocery queries. That's the nature of what was asked for (there's no on-device OCR here); it's the user's own image, their own Groq account, for their own single-user tool.

**Upload is staged, not fired on file-pick — this was a real usability bug, not a style choice.** The first version called `import-image` the instant a file was chosen. Reported by the user directly: they had no way to see what was about to be sent, no chance to add context, and it happened before they'd decided to send anything. Now `onImageChosen` only downscales the file and sets `pendingImage`; a preview strip (`.pending-attachment`) shows the real thumbnail plus an × to cancel, and the text input's placeholder switches to "Add a note (optional)…". Only submitting the form (Send, or Enter) actually calls `import-image`, with whatever note was typed. That note is threaded all the way to `extractItemsFromImage(dataUrl, instructions)` and appended to the vision prompt as a literal instruction ("only get the snacks") — not cosmetic. The attach button itself went through two rounds: an emoji (rejected, "not aesthetic"), then a photo-icon SVG that rendered as a blank block because the generic `.chat-input-row button` primary-CTA rule was overriding it and the SVG had no explicit size — the final version is a circular "+" button scoped under `.chat-input-row .chat-attach-btn` specifically to beat that rule, sized to match the input/Send row.

### 6.16 Web search — recipe grounding and the per-item "Explain" popup

**Why this exists.** Every LLM call in this app up to this point answers from training-data recall alone — fine for "what's the search term for X," a real gap for "what does biryani actually need" or "is this specific product good for Y," where a wrong or outdated answer is a real cost (an ingredient genuinely missing from a recipe, an invented nutrition claim about a real product). The user asked for two features built on the same new capability: ground the recipe proposal in a real recipe instead of pure recall, and let the user ask free-form questions about one specific product with real web content behind the answer.

**Provider choice: Tavily, not a raw search API.** This needed picking between Tavily, Brave Search, and Google Custom Search — all viable, but Tavily is built specifically for feeding LLM agents: its `/search` response already returns clean, pre-summarized `content` per result (and an optional synthesized `answer` field) rather than raw HTML needing its own scrape-and-clean step, which is what actually decided it. `backend/src/agent/webSearch.js`'s `searchWeb({query, maxResults})` is a thin wrapper — POST to `api.tavily.com/search`, `Authorization: Bearer <key>`, `search_depth: "basic"`, `include_answer: "basic"` — verified against Tavily's live API reference before writing it, same "no hallucinated params" discipline this file already holds for Swiggy's own tools.

**Fail-open by construction, not by accident.** `TAVILY_API_KEY` is optional (`config.tavilyApiKey`, blank by default) precisely so a fresh clone of this repo — or a session where Tavily itself is down — still has fully working recipe proposal and Explain, just without the grounding upgrade. `searchWeb` returns `null` for a missing key, a non-2xx response, or a network error — it never throws — and every caller treats `null` as "no grounding available," not a failure to propagate. Verified live end-to-end with no key configured: `propose_ingredients` for "pasta" produced its normal ungrounded checklist (penne, tomato ketchup, olive oil, garlic, onion, green chilli, basil leaves, parmesan, salt) with no grounding note shown, and the Explain popup answered a real question ("is this good for weight loss?") by honestly saying it lacked specific nutrition data rather than inventing numbers — exactly the behavior the system prompt asks for when the given content doesn't cover the question.

**Recipe grounding — "search first, then generate," a genuine second completion.** This was an explicit choice among three options the user was asked about: let the model decide when to search (more flexible, less predictable, and re-introduces the multi-completion slowness §6.3 fixed); ground always via one combined completion; or search unconditionally then re-derive the list in a dedicated second completion. The chosen design keeps completion #1 exactly as it was — the model still recognizes the recipe intent and dish name via its own `propose_ingredients` call, and that call's own ingredient list becomes a **fallback only**. `webSearch({query: "${dish} recipe ingredients"})` runs next, and if it returns anything, `groundIngredients(dish, search)` — a bounded, single-purpose completion in the same style as `discoveryAgent.js`'s `judgeRelevantItems` (forced `tool_choice`, `reasoning_effort: "low"`, not routed through `runToolLoop` since there's no multi-turn tool use here) — re-derives the list from the real recipe content, with the *same* Indian-grocery-naming system-prompt rules as the original tool (§6.13's "curd not yogurt" list). A successful regrounding sets `grounded: true` and up to 3 source URLs, surfaced in the checklist as a small note; any failure at any step (no results, malformed completion, non-tool-call finish reason) silently keeps the original fallback list.

**The grounding search itself wasn't biased toward India at all — this app is explicitly for an Indian catalogue, and the un-grounded "pasta" example two paragraphs up (parmesan, olive oil, basil leaves — a Western ingredient list) is exactly the failure this closes, not just an illustration of fail-open behavior.** A dish name like "pasta" or "sandwich" has both an Indian and an international identity, and a bare `"${dish} recipe ingredients"` search has no reason to prefer one over the other — confirmed live, it returned generic (Italian-style) results. Fixed with Tavily's own `country` parameter — verified against Tavily's live API reference (boosts results toward a given country, valid only when `topic` is the default `"general"`, which this app never overrides) — threaded through `searchWeb({query, maxResults, country})` in `webSearch.js` and passed as `country: "india"` from the recipe-grounding call, alongside changing the query text itself to `"${dish} Indian recipe ingredients"`. Confirmed live: the same "pasta" request now grounds in real Indian food-blog recipes (indianhealthyrecipes.com, jcookingodyssey.com, subbuskitchen.com) and produces an authentic "masala pasta" list — cumin seeds, garam masala, kasuri methi, capsicum — instead of the Western default. Both system prompts involved (the ungrounded `propose_ingredients` tool description, and `groundIngredients`'s own system message) were also reinforced beyond "use Indian grocery *names*" to "this must be the authentic Indian home-cooking *version* of the dish" — the naming fix alone doesn't help if the underlying recipe itself is a Western one with Indian-sounding staple names swapped in. Deliberately scoped to the recipe-grounding search only (not the per-item Explain search, which isn't about recipes) since that's what was actually asked for.

**One term was deliberately left unchanged despite looking like an obvious candidate: "bay leaf."** The user's own framing of this whole fix used "bay leaf" as the motivating example of an item "not heard of in India." Checked directly against the live catalogue (see the `recipe_2.png` screenshot already in the README) before touching anything: searching "bay leaf" on Instamart already returns "ZOFF Premium Indian Bay Leaf" — the Indian tej patta product is sold under that exact English name on real Indian packaging. Renaming it would have fixed nothing and risked breaking an already-correct case; the real gap was upstream (the search wasn't biased toward Indian recipe sources at all), not in this one term's translation.

**A real, pre-existing tool-calling reliability gap in this same flow, found while testing the above and fixed as a follow-up — root-caused to `tool_choice: "auto"`, mitigated but not fully eliminated.** Repeated identical "order things for making biryani" requests sometimes never called `propose_ingredients` at all: the model would answer with a plain-text ingredient list instead (no checklist UI, nothing addable), or — once — call `get_cart`/`search_products` instead. Reproduced consistently enough (roughly 1 in 3) to rule out a fluke, and confirmed via `git stash` that this predates every change in this session, i.e. genuine pre-existing Groq/model flakiness on `tool_choice: "auto"`, the setting `runToolLoop` (`toolLoop.js`) had always used unconditionally. `runToolLoop` gained an optional `forceToolName` param — forces that exact tool via `tool_choice` on iteration 0 only (reverting to `"auto"` from iteration 1 on, in case a forced call somehow doesn't end the turn), with a graceful fallback to unforced `"auto"` if Groq 400s with its known "Tool choice is required, but model did not call a tool" error (the same failure mode already handled for Explain's forced `answer_question` call, §6.16-ish above) rather than aborting the whole turn. A new deterministic pre-gate, `looksLikeRecipeRequest()`, sits alongside `looksOffTopic` with the identical high-precision philosophy — it only fires on the phrasings the app's own `SYSTEM_PROMPT` already uses as its canonical examples ("order things for biryani", "I want to make pasta") plus a few obvious variants ("help me cook X", "ingredients for X", "recipe for X", "how do I make X") — verified against 20+ positive and negative example phrasings before wiring it in, so a plain shopping message like "add milk" or "i need some snacks" can never misfire into forcing the wrong tool. `sendMessage` passes `forceToolName: "propose_ingredients"` only when this pre-gate matches.

Measured, not assumed: repeated live batches against the real account went from a consistent ~33% failure rate (the original, unforced code, reproduced via `git stash` specifically to isolate this from every other change in the same file) to two separate batches at 87% and 90% success once forcing was in place — a real, substantial improvement, confirmed by directly inspecting one of the rare remaining failures (a fast ~5s response with plain text, not a slow timeout, and no forced-tool_choice 400 in the logs — meaning Groq's own forced-tool_choice contract wasn't honored that one specific time, which per OpenAI-compatible tool-calling semantics should essentially never happen, but Groq isn't perfectly consistent about it). This is **not claimed as a full fix** — a small residual failure rate remains and would need a Groq-side explanation (or an additional forced retry loop) to close completely. A clean, larger-sample final number wasn't obtained because the verification process itself ran out of runway — see the token-quota-exhaustion note below, an unplanned but real finding from the same testing session.

**Testing this uncovered a second, distinct rate-limit failure mode: Groq's real per-day token quota, not just the per-minute one already documented in §6.3.** Running roughly 60 recipe-grounding requests back-to-back to get a reliable before/after sample (each costing ~1700-2000+ tokens, more when grounding's second completion runs) tripped a hard `429` on `openai/gpt-oss-120b`'s **daily** token limit (`Limit 200000, Used 198518`) — every subsequent request failed identically regardless of `createCompletionWithRetry`'s existing 429-retry logic, because that logic assumes a short, recoverable per-minute window (§6.3's documented ~8000 TPM ceiling), not a genuinely exhausted daily budget. Distinct in a way worth remembering: the existing TPM retry recovers in a couple of seconds; this one doesn't recover until enough of Groq's rolling daily window frees up on its own (their `retry-after` header reported a few minutes, not a full day-boundary reset, so it's evidently a rolling window rather than a hard midnight cutoff) — and during that window, every Groq-backed feature in the app (chat, recipes, Explain) fails identically, not just the one being tested. Worth knowing before a demo, or before running a large manual test batch against a real account: this is a hard ceiling, not something retry logic can paper over.

**Per-item Explain, its own overlay.** The other explicit choice: should the answer live in the main cart-building chat, or its own separate popup? The user chose separate — researching a product ("is this good for X") is a different activity from building an order, and mixing the two would clutter the one transcript that matters for cart-building. `ExplainModal` is entirely frontend state (its own `history` array, unmounted on close) — nothing about it is persisted server-side, unlike the main chat's `displayTranscript`. `explainItem` is built from the product's own known details (name/brand/size/price, sent by the frontend from the same card data already rendered) plus web content plus the last 6 turns of history the frontend resends each time (no server-side conversation state to manage).

**Retrieval went through three designs; the first two each shipped a bug the user caught.** (a) *One search per item, reused for every question* (`"${name} ${brand} ingredients nutrition review"`), on the theory that the product doesn't change mid-conversation so re-searching is waste. Wrong in practice: "does this have caffeine in it?" about Coca-Cola Zero was flatly refused, because that one generic search never surfaced caffeine and the prompt told the model to use only what it was given. (b) *A fresh, question-specific search each turn, discarded afterwards.* That fixed caffeine but broke something worse — each question started blind, so a fact retrieved for one question was invisible to the next. The reported symptom: "does this have sugar content/" → "I don't have specific information", then two answers later "…only 2 g of sugar". An assistant contradicting itself inside one conversation is a worse failure than a slow one. (c) **Current: an accumulating per-item pool** (`itemKnowledge`). Each question still runs its own targeted search, but results are *added* to the item's pool and every answer is built from the whole pool — knowledge is monotonic within a session, so "it knew that a moment ago" is structurally impossible. Opening an item also fires one **baseline** `nutrition facts ingredients specifications` search, so the core factual panel is present no matter how a given question is worded (the single biggest source of phrasing-dependent inconsistency on food items). Queries are built from **stopword-stripped** terms (`explainSearchTerms`) because a chat-box question is a poor search string verbatim — "does this have sugar content/", trailing slash and all, becomes `sugar content`. And if the model still reaches `insufficient_info`, one **broader retry** search runs before it's allowed to refuse, adopted only if it actually beats the first attempt (never downgrading a usable answer into a refusal). Measured after the change: first question ~6s (baseline + targeted search + completion), follow-ups ~1s off the warm pool.

**Search results are relevance-filtered in code, and Tavily's `answer` summary is not trusted on its own — it fabricates.** This was found by probing with a deliberately fictional product, "Zzyqx Blorptastic Nutrient Cube 4400". Tavily returned three pages about unrelated snacks *and* a synthesized summary stating the fictional product "contains 200 mg of sodium per serving… part of the Zee Zees brand" — a number lifted from an unrelated Zee Zees nutrition page. The model then reported that as fact **3/3 times even after the system prompt was updated to explicitly warn that sources may describe other products**, which is the whole argument for not solving this with prompting: `explainMentionsProduct` is a deterministic gate, the same reasoning as `filterRelevantProducts` (§6.10) for Swiggy's own fuzzy search. A source enters the pool only if it genuinely mentions this product, keyed primarily on **brand** — brands are distinctive where "protein"/"bar"/"milk" are not — with a normalized substring match so punctuation differences ("Dr. Sheth's" vs "Dr Sheth") don't cause false misses. The subtle part: **running that same check on Tavily's summary is useless**, because the summary parrots the query's own wording back and therefore always "mentions" the product even when wholly invented. So the summary is admitted only if **at least one real source page passed the filter** — it's synthesized from those pages, so if none of them are about this product, neither is it. Verified after the fix: the fictional product refuses 3/3 with zero sources, while RiteBite/Coca-Cola/Amul still answer with exact figures and three citations each. The filter is deliberately strict, because the error costs are asymmetric — a false negative degrades to general knowledge or an honest "I don't know", a false positive invents a nutrition figure for the wrong product.

**The forced tool call needs a fallback path.** Forcing `answer_question` guarantees `basis` is always reported, but Groq intermittently rejects a forced tool call outright with `400 "Tool choice is required, but model did not call a tool"` and an empty `failed_generation` — hit on **2 of 4** consecutive real questions during verification, surfacing to the user as a 500 from the route. On that specific error only, `explainItem` retries once in JSON mode (`response_format: { type: "json_object" }`) asking for the same `{answer, basis}` shape — the same mechanism `imageImport.js` already depends on — rather than failing the request. Verified 0 HTTP errors across a multi-turn conversation afterward.

**General-knowledge fallback, reported honestly rather than silently blended.** Fixing the search alone wasn't enough on its own — the underlying design flaw was a system prompt that told the model to *only* use the given content and refuse otherwise, which is too strict for an assistant that's allowed to know things. `explainItem` now forces a tool call, `answer_question(answer, basis)`, with `basis` constrained to `"search" | "general_knowledge" | "insufficient_info"` and an explicit priority order in the prompt: real search content first (most reliable, specific to this exact listing); failing that, the model's own well-established knowledge rather than a refusal (never invent a *specific* number or claim it isn't sure of, but don't refuse a widely-known fact either); only `insufficient_info` when neither source is confident. The backend never trusts the model's own `"search"` self-report at face value — `grounded = basis === "search" && the search actually returned content` — the same "don't trust a self-report as proof of what really happened" discipline as the cart-verification code (§6.6, §6.9). The frontend renders the two cases differently on purpose: a `search`-basis answer gets a "Sources: [1][2][3]" line with real links; a `general_knowledge`-basis answer gets a quieter, dashed-border "💭 General knowledge — not specific to this listing" note instead — so a cited claim and a recalled one are never visually indistinguishable, letting the user calibrate how much to trust each one.

**Scope, deliberately narrow.** Explain only ships on Insta-nt's `ProductCard` for now, not Feaster's dish results — the user's own framing was in terms of "items" (Instamart's product vocabulary), and Feaster already has its own (separately-labeled, LLM-*estimated*) nutrition guess; extending Explain there is a distinct, later task, not bundled into this one.

**The modal must be portalled — a CSS containing-block trap, found in production.** The first version rendered `<ExplainModal>` inline inside `<ProductCard>`'s JSX, which put the `position: fixed` overlay inside `.product-card`'s DOM subtree. That card sets `overflow: hidden` and, on `:hover`, `transform: translateY(-2px)` — and **a transform makes an element the containing block for its `position: fixed` descendants**, so `inset: 0` resolved against the ~200px-wide card rather than the viewport, then got clipped by the overflow. The card is necessarily hovered at the moment its own button is clicked, so this fired every single time. Reported by the user with a screenshot showing the dialog as a squeezed column of one-word-per-line text wedged inside the product grid; verified after the fix that `overlay.parentElement === document.body` and the overlay measures exactly the viewport (1280×720). `createPortal(…, document.body)` is the fix, and it's the reason the modal is not simply styled its way out of the problem — no z-index or positioning tweak can escape a containing block. The same rebuild added the standard modal affordances that were missing: Escape-to-close, background scroll lock (restored on unmount), backdrop blur, a click-outside-to-close backdrop, tap-to-ask suggestion chips for the empty state, and animated typing dots reusing the existing `typing-bounce` keyframe.

**Grounding regressed an existing localization rule, and the fix is deterministic.** §6.13's `propose_ingredients` prompt has always instructed Indian grocery naming ("curd" not "yogurt", "coriander leaves" not "cilantro") because it materially changes what Instamart returns. Grounding broke that: the model is now handed real recipe pages, which are overwhelmingly US-English, and it follows the wording in front of it over an instruction further up the prompt. Confirmed live from a user report — a grounded list emitted "plain yogurt", and the auto-add picked **Epigamia Greek Yogurt – Raspberry** for it. Verified the cause directly by running both searches against the live catalogue: `"plain yogurt"` returns Greek/flavoured tubs (Milky Mist Greek Yogurt, Epigamia Natural/Blueberry/Mango), `"curd"` returns plain dahi (Amul Masti Dahi, Milky Mist Set Curd). Rather than strengthening the prompt again (the same lever that just failed), `toIndianGroceryTerm` applies a deterministic term map inside `dedupeIngredients`, so it covers the grounded list *and* the ungrounded fallback with no extra model call — consistent with this codebase's standing preference for code over model judgment wherever there's exactly one right answer. It runs **before** dedupe so "curd" and "plain yogurt" in the same list collapse into one entry instead of ordering the ingredient twice. The map is intentionally small and unambiguous-only; bare "coriander" is excluded because it can mean either the powder or the leaves, and guessing there would trade one wrong item for another.

**Auto-pick logic originally shared the recipe flow's `sortForBestPick`/`orderBestFirst` layer (§6.13) — it no longer does, after two live bugs surfaced that only make sense to fix separately.**

*Bug 1 — the size-matching code had been comparing against `undefined` since it was written.* `flattenVariants`, the shared function every ranked/filtered variant list in this file is built from, never attached a variant's `quantityDescription` to the objects it produced — that field was only ever added later, after sorting, by `enrichProducts` pulling from the display cache. So `quantityMatches(requested, v.quantityDescription)` — the exact-size check §6.14 relies on — was silently evaluating `parseQuantityFrom(undefined)` for every single variant, always `null`, always `false`. The user-visible effect: a real 5-item screenshot import reported "Added 0 exact matches to your cart; 5 need you to pick" even though a literal exact match existed for most of them (confirmed live: a genuine "Coca-Cola Diet Coke Can 330 ml" at ₹48 sitting in the search results for a 330 ml Diet Coke import, never picked up). One-line fix: `flattenVariants` now also sets `quantityDescription: v.quantityDescription || null`.

*Bug 2, surfaced only once bug 1 was fixed and real size data started flowing through* — with exact matches now working, one of them auto-added a ₹290 two-pack of chicken instead of a ₹145 single pack, both genuinely exact-size matches. Not a data bug this time: this real account's `your_go_to_items` has *separate, individually-ranked entries per pack size* of the same product (confirmed live: a 6-pack, 4-pack, 2-pack, and single can of Coca-Cola Zero each carry their own distinct rank), and the account's real order history happens to rank the bigger packs higher. `orderBestFirst`'s existing rule — "most ordered by you wins even if pricier," a deliberate, previously-confirmed app-wide default — took that at face value even though the screenshot only ever showed one single item.

Asked directly which behavior the import flow specifically should have (not changing the app-wide default, which the recipe flow still uses and which has no per-item size to rank against anyway): the user's answer was that closeness to what the screenshot actually showed should win here, with "most ordered by you" downgraded to a tiebreak. Implemented as a new function, `sortForImportPick(variants, requested)`, used only inside `confirmImportDirect` for both jobs — picking which exact match auto-adds, and ordering the "no exact match, pick one" swap options. Priority order: `|own pack size − requested size|` ascending (reusing `parseQuantityFrom`'s existing per-pack parsing, so a 6-pack of 300 ml cans compares on 300 ml, not 1800 ml) → pack multiplier ascending (a plain single unit beats an otherwise-identical multipack) → mostOrdered/orderRank → price, each only as a tiebreak for the one before it. `sortForBestPick`/`orderBestFirst` are otherwise untouched and still back the recipe flow (§6.13) exactly as before.
