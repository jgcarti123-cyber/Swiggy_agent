...
| LLM | Groq (`groq-sdk`), text model `openai/gpt-oss-120b` + vision model `meta-llama/llama-4-scout-17b-16e-instruct` | Text model powers restaurant-discovery judgment calls and free-text interpretation in the Instamart chat (see §5); the vision model is used only by the screenshot-import feature (§6.15), since the text model has no image input |
...
**Why a second model.** The app's default model (`openai/gpt-oss-120b`) has no image input, so this uses a separate vision-capable model (`config.groqVisionModel`, default `meta-llama/llama-4-scout-17b-16e-instruct` — confirmed present in both Groq's vision docs and the live models table before adopting it; it's a preview model, hence the env override). It's isolated in its own module (`agent/imageImport.js`) with one job — `extractItemsFromImage(dataUrl)` returns `[{ name, size, quantity }]`. The prompt is deliberately narrow: product line items only (brand + generic product name, pack size, quantity from the stepper), everything else on a busy checkout screen — prices, "you saved ₹164" banners, delivery ETA, UPI offers, address, totals, buttons — explicitly ignored. `response_format: json_object`, `temperature: 0`, and a salvage-the-first-`{…}`-block fallback in case the model wraps its JSON in prose. Verified live against the user's actual screenshots: a 4-item cart came back as clean, search-ready terms with correct sizes and quantities.
