...
**Per-item Explain — one search, many questions, its own overlay.** The other explicit choice: should the answer live in the main cart-building chat, or its own separate popup? The user chose separate — researching a product ("is this good for X") is a different activity from building an order, and mixing the two would clutter the one transcript that matters for cart-building. `ExplainModal` is entirely frontend state (its own `history` array, unmounted on close) — nothing about it is persisted server-side, unlike the main chat's `displayTranscript`. The backend side is stateless per call except for one cache: `itemSearchCache`, a module-level `Map` keyed by spinId (same session-lifetime pattern as `productBySpin`/`knownOutOfStockIds`), storing the search result (or `null`, so a failed search isn't retried on every follow-up either) from the *first* question asked about that product. Every subsequent question — same modal session, or a fresh one opened later for the same product — reuses that cached search instead of re-searching, so only the first question per item pays the search latency; every question after that is a single plain completion (`explainItem`, no tool-calling — this is open Q&A, not a structured extraction) built from the product's own known details (name/brand/size/price, sent by the frontend from the same card data already rendered) plus the cached web content plus the last 6 turns of history the frontend resends each time (no server-side conversation state to manage). The system prompt explicitly instructs honesty over fabrication: if the given content doesn't answer the question, say so rather than inventing ingredients, nutrition facts, or claims.
