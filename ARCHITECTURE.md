...
- **`InstamartChat.jsx`** — renders the chat transcript, including four message types beyond plain text: `choice` (clickable brand chips, including the dashed "Any brand" option, §6.4), `products` (an image-card grid, each with a price, an Add button, and a ☆ save-to-usuals toggle, out-of-stock ones visibly marked and disabled, §6.5/§6.11), `ingredients` (the recipe checklist, §6.13 — chip edits are local component state until Confirm), and `recipe` (compact per-ingredient swap rows, §6.13). Each card's "Add" and ☆ call deterministic direct-action endpoints, not the chat endpoint; a saved item's star fills (★) driven by a `savedKeys` set held in the panel. Owns the usuals + schedule state and renders `UsualsPanel` in the right column under the cart. The chat canvas is near viewport height and takes the wide grid column (2.2fr vs 1fr rail); while a request is in flight the typing dots carry a rotating status line (`THINKING_LINES`, advances ~2s, parks on the last line so it never loops like a hang).
