...
**Step 1 — the model proposes, inside its tool call.** A new LLM tool, `propose_ingredients(dish, ingredients[])`, has the model generate the essential-ingredient list *in the tool-call arguments themselves* — there is no second "planning" completion, no search results fed back for judgment. The system prompt constrains the list to short, generic, brand-free Instamart search terms. `executeTool` sanitizes (trim/dedupe/cap 16) and ends the loop via the `__endLoop` sentinel (§6.1) with a new branch kind, `ingredients` — rendered as an editable checklist (chips with ×, an add-ingredient input, a Confirm button). The user, not the model, owns the final list.

**Step 2 — Confirm is fully deterministic** (`confirmRecipeDirect`, `POST /recipe-confirm`, zero Groq calls). The confirmed list fans out through `mapWithConcurrency(…, 3)` — the same throttle-don't-Promise.all reasoning as Feast Finder's menu fan-out — and each ingredient runs the *identical* pipeline as a normal guided search: relevance filter (§6.10), go-to cross-reference (§6.9), stock/price sort (§6.5), image side-channel (§6.7). Per ingredient: up to 3 **in-stock** options are kept, and the best (first after sorting) is auto-added — all bests in one `addUsualsBestEffort` batch (§6.11), so the reply's "Added N of M" counts come from the verified real cart, never call success. Ingredients with zero relevant in-stock results render as "couldn't find" and are named in the reply; one ingredient's search failure can't sink the rest.

**The options render compact by design** — a new `recipe` transcript type with small thumb-rows (38px), not the big product-card grid: a 12-ingredient result would otherwise be ~36 full cards of scrolling. The auto-added pick is highlighted "✓ In cart"; every other row has a Swap button.
...
**Known limitation, inherited deliberately:** the keyword relevance filter can't tell "mint leaves" from "POLO Breath Mint" (§6.10's literal-word-overlap trade-off) — observed live: the mint auto-pick was a breath mint, chicken's was a marinade mix. The swap rows are the designed recovery path (real mint/chicken alternatives were right there); fixing it "properly" would mean an LLM relevance pass per ingredient, i.e. reintroducing exactly the per-search completions this feature was built to avoid.
